...
public class UserController {
	...
	@GetMapping("/isVerify")
	public ResponseEntity<?> isVerify(Principal user) {
		try {
			return ResponseEntity.ok(new DefaultResponse(userService.findUserByPhoneNum(user.getName()).getVerify(), ""));
		}
		catch (Exception e) {
	...
	@PostMapping("/block")
	public ResponseEntity<?> block(@RequestBody String cardNum) {
		try {
			...
			return ResponseEntity.badRequest().body("Error 3");
		}
	}

	@PostMapping("/unblock")
	public ResponseEntity<?> unblock(@RequestBody String cardNum) {
		Card card = cardService.findCardByCardNum(cardNum);
		card.setLock(false);
		cardService.save(card);
		return ResponseEntity.ok(new DefaultResponse("Successful", ""));
